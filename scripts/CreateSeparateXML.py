...
Input files
-------------
*``combined_prot_aligned.fasta`` : file for each protein that contains aligned human and swine influenza protein sequences used for building trees
*``tail.xml`` : bottom portion of XML that contains parameters for BEAST

Output files
-------------
*``head.xml`` : file generated for each protein containing top portion of XML file
*``beastinfile.xml`` : complete XML file generated for each protein 
...
<beast>\n\n\
	...
	for seq in sequences:
        ...
        print 'checking length %s' % len(seq.seq)
        print seq.seq

	f.write('	</taxa>\n\n\
	<!-- The sequence alignment (each sequence refers to a taxon above).         -->\n\
	<!-- ntax=%s nchar=%s                                                      -->\n\
	<alignment id="alignment" dataType="amino acid">\n' % (len(sequences), len(sequences[0].seq)))
...
def main():
    ...
    for sequence in sequencesets:
        ...
        xmlinfile = '%sseparatebeastinfile.xml' % base
